...
export default function Layout({page,setPage,children}){
  const{HEROES,estimerMission,owned,team,teamPresets,activeTeamSlot,selectTeamPreset,teamPower,gems,gold,hearthstones,bloodFragments,forgeEssence,summonerProfile,summonerXpRequired,setSummonerName,unlocks,battleSession,battleInProgress,pendingMission,preparationMission,cancelMissionPreparation,confirmMissionPreparation,abandonBattle,dismissPendingMission,replaceBattleWithPending}=useGame();
  const[prepTeam,setPrepTeam]=useState(team);
  // Estimation par simulation : on joue vraiment la mission plusieurs fois
  // plutot que d'annoncer un nombre de puissance qui se trompe de 30 %.
  const[estimation,setEstimation]=useState(null),[estimeEnCours,setEstimeEnCours]=useState(false);
  const estimer=()=>{setEstimeEnCours(true);setEstimation(null);
    window.setTimeout(()=>{try{setEstimation(estimerMission(preparationMission,prepTeam))}
      catch(erreur){console.error('Estimation impossible',erreur);setEstimation({echec:true})}
      finally{setEstimeEnCours(false)}},20)};const preparationMaxMembers=Math.max(1,Math.min(4,Number(preparationMission?.teamSize)||3));
  useEffect(()=>{if(preparationMission)setPrepTeam([...team])},[preparationMission]);
  // Une nouvelle equipe invalide l'estimation precedente : elle ne parle plus
  // du combat qu'on s'apprete a lancer.
  useEffect(()=>{setEstimation(null)},[preparationMission,prepTeam.join(',')]);
