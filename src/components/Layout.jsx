...
export default function Layout({page,setPage,children}){
  const{HEROES,owned,team,teamPresets,activeTeamSlot,selectTeamPreset,teamPower,assessMission,gems,gold,hearthstones,bloodFragments,forgeEssence,summonerProfile,summonerXpRequired,setSummonerName,unlocks,battleSession,battleInProgress,pendingMission,preparationMission,cancelMissionPreparation,confirmMissionPreparation,abandonBattle,dismissPendingMission,replaceBattleWithPending}=useGame();
  const[prepTeam,setPrepTeam]=useState(team);
  // « Estimer mes chances » a ete retire : annoncer « tu gagnes 17 fois sur 20 »
  // avant d'appuyer sur Lancer supprimait la seule question qui fait qu'un
  // combat vaut la peine d'etre joue. Le simulateur reste dans le depot comme
  // outil de mesure d'equilibrage — il n'est simplement plus expose au joueur.
  //
  // Ce que cette fenetre montrait en revanche, c'etait DEUX NOMBRES : la
  // puissance de l'equipe et une puissance recommandee. Or aucun nombre ne peut
  // dire qu'il manque un soigneur — et c'est ce qui decide. Mesure : contre le
  // raid, une equipe composee (un soin, un bouclier, deux frappeurs) gagne la ou
  // l'equipe des quatre champions LES PLUS PUISSANTS perd, avec jusqu'a trois
  // fois moins de puissance affichee.
  //
  // On affiche donc les MANQUES de composition, deja calcules par
  // `assessTeamForMission` et jusqu'ici visibles seulement sur les ecrans de
  // selection — pas ici, ou l'on compose. C'est une aide a la decision, pas une
  // reponse : le joueur apprend qu'il lui manque un soin, pas s'il va gagner.
  ...
  const manques=preparationMission&&prepTeam.length
    ?(assessMission(preparationMission,prepTeam)?.gaps||[]):[];
