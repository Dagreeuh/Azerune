...
const tryDebuff=(actor,target,key,turns,baseChance,mastery,resisted)=>{
  ...
  if(Math.random()<=chance){target.debuffs[key]={turns,source:actor.id};return true;}
...
export function nextTurn(battle){
  ...
  if(unit.debuffs.burn){const amount=Math.round(unit.maxHp*.05*(unit.setEffects?.includes('fireproofSet')?.75:1));dot+=amount;periodic.push(`Brûlure ${amount}`);}
  if(unit.debuffs.bleed){const amount=Math.round(unit.maxHp*.045);dot+=amount;periodic.push(`Saignement ${amount}`);}
  ...
  if(unit.debuffs.corruption){const amount=Math.round(unit.maxHp*.035);dot+=amount;periodic.push(`Corruption ${amount}`);}
