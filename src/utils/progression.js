...
export const remainingResonanceFragments=progress=>RESONANCE_COSTS.slice(Math.max(0,Math.min(MAX_RESONANCE,Number(progress?.resonance||0)))).reduce((sum,value)=>sum+value,0);
...
export function normalizeChampionProgress(hero,value){const fallback=defaultChampionProgress(hero),stars=Math.max(hero.rarity,Math.min(MAX_STARS,Number(value?.stars??fallback.stars))),level=Math.max(1,Math.min(levelCap(stars),Number(value?.level??1)));return{stars,level,xp:Math.max(0,Number(value?.xp??0)),soulFragments:Math.min(remainingResonanceFragments({resonance:Number(value?.resonance??0)}),Math.max(0,Number(value?.soulFragments??0))),resonance:Math.max(0,Math.min(MAX_RESONANCE,Number(value?.resonance??0)))};}
