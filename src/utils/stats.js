...
// Puissance recommandee d'une rencontre.
//
// Quand le mode annonce lui-meme une puissance calibree, c'est elle qui fait
// foi : la deriver des statistiques ennemies donnerait un second chiffre,
// different, sur le meme ecran. C'est arrive au Mythic+, dont la puissance
// recommandee est ajustee sur la puissance reelle des joueurs alors que la
// jauge de preparation affichait une valeur trois fois superieure, recalculee
// depuis des ennemis dont l'Attaque venait d'etre multipliee par vingt.
//
// Le predicat ci-dessous enumerait les modes par PROCURATION — « a-t-elle un
// difficultyId ? est-ce du Mythic+ ? » — au lieu de poser la seule question qui
// compte : cette mission annonce-t-elle un chiffre ? Le Raid et les Expeditions
// en annoncent un, releve par simulation, et le predicat les excluait faute de
// `difficultyId`. Le defaut meme que ce commentaire decrit s'etait donc
// reproduit dans deux modes de plus, en pire : l'ecran de raid affichait
// « Recommandee : 10 200 » pendant que le verdict a cote etait calcule sur
// 52 300 — un facteur CINQ entre deux chiffres de la meme page.
const ANNONCE_CALIBREE=mission=>Number.isFinite(Number(mission?.recommended));
export function calibratedEncounterPower(mission={}){if(ANNONCE_CALIBREE(mission)&&Number.isFinite(Number(mission.recommended)))return Math.max(1,Math.round(Number(mission.recommended)));const enemies=mission.enemies||mission.waves?.[0]||[],raw=enemies.reduce((sum,enemy)=>sum+enemyPower(enemy,1),0),kind=missionKind(mission),boss=Boolean(mission.boss||mission.raid||mission.mythic),mechanics=(mission.raidData?.mechanics||mission.expeditionData?.mechanics||mission.mechanics||[]).length,waves=mission.waves?.length||1,affixes=mission.affixIds?.length||mission.affixes?.length||0,level=mission.raidLevel||mission.expeditionLevel||mission.mythicLevel||Number(mission.stageId)||1;let factor=boss?1.48:1.35;if(kind==='expedition')factor*=1.12+level*.025+mechanics*.055;if(kind==='raid')factor*=1.48+level*.055+mechanics*.10;if(kind==='mythic')factor*=1.22+(waves-1)*.16+affixes*.14+level*.012+Math.floor(level/10)*.12;return Math.max(1,Math.round(raw*factor));}
