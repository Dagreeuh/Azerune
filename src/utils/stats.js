...
const SKILL_TAGS={
 heal:['heal','healLow','cleanseHeal','teamHeal','teamRegen','healingSeed','seedBloom','livingGarden','atonementStrike','atonementPenance','totemHeal','healingTotem','totemTide','soulMetamorphosis'],
 cleanse:['cleanse','cleanseHeal','healingSeed','seedBloom'],
 shield:['shield','singleShield','guardianLink','guardianWall','rescueShield','rescueSanctuary','atonementShield','soulMetamorphosis'],
 control:['stun','slow','slowDrain','provoke','turnMeter','impactQuake','unstableStun','gardenPrison','refluxStrike','refluxDrain','bladeDanceDrain','highTide','soulSigil'],
 aoe:['allEnemies'],
 debuff:['defDown','atkDown','poison','burn','bleed','agony','corruption','festeringStrike','festeringSpread','impactFracture','herbalThorn','lowTide','condemnStrip','alchemyMix','alchemyCatalyst']
};
const hasAny=(text,values)=>values.some(value=>text.includes(value.toLowerCase()));
function heroCapabilities(hero){const skills=hero?.skills||[],effects=skills.map(skill=>String(skill.effect||'').toLowerCase()),targets=skills.map(skill=>String(skill.target||''));const text=effects.join(' ');return{heal:hasAny(text,SKILL_TAGS.heal),cleanse:hasAny(text,SKILL_TAGS.cleanse),shield:hasAny(text,SKILL_TAGS.shield),control:hasAny(text,SKILL_TAGS.control),debuff:hasAny(text,SKILL_TAGS.debuff),aoe:targets.includes('allEnemies'),support:targets.some(value=>['ally','allAllies','self'].includes(value))};}
function missionKind(mission={}){return mission.raid?'raid':mission.mythic?'mythic':mission.expedition?'expedition':'campaign';}
export function calibratedEncounterPower(mission={}){if(mission.difficultyId&&Number.isFinite(Number(mission.recommended)))return Math.max(1,Math.round(Number(mission.recommended)));const enemies=mission.enemies||mission.waves?.[0]||[],raw=enemies.reduce((sum,enemy)=>sum+enemyPower(enemy,1),0),kind=missionKind(mission),boss=Boolean(mission.boss||mission.raid||mission.mythic),mechanics=(mission.raidData?.mechanics||mission.expeditionData?.mechanics||mission.mechanics||[]).length,waves=mission.waves?.length||1,affixes=mission.affixIds?.length||mission.affixes?.length||0,level=mission.raidLevel||mission.expeditionLevel||mission.mythicLevel||Number(mission.stageId)||1;let factor=boss?1.48:1.35;if(kind==='expedition')factor*=1.12+level*.025+mechanics*.055;if(kind==='raid')factor*=1.48+level*.055+mechanics*.10;if(kind==='mythic')factor*=1.22+(waves-1)*.16+affixes*.14+level*.012+(level%10===0?.12:0);return Math.max(1,Math.round(raw*factor));}
export function assessTeamForMission(members,heroes,getStats,mission={}){const chosen=(members||[]).map(id=>heroes.find(hero=>hero.id===id)).filter(Boolean),stats=chosen.map(getStats),caps=chosen.map(heroCapabilities),recommended=calibratedEncounterPower(mission),power=teamPower(members,heroes,getStats),ratio=power/Math.max(1,recommended),avg=key=>stats.length?stats.reduce((sum,value)=>sum+(value[key]||0),0)/stats.length:0,has=key=>caps.some(value=>value[key]),allEffects=stats.flatMap(value=>value.setEffects||[]),kind=missionKind(mission),fireThreat=kind==='raid'||mission.enemies?.some(enemy=>enemy.element==='Feu')||false,fireproof=allEffects.includes('fireproofSet'),needsSustain=kind==='raid'||kind==='mythic'||Boolean(mission.boss),needsCleanse=fireThreat||mission.affixIds?.includes('necrotic'),needsAoe=kind==='raid'||kind==='mythic'||(mission.enemies?.length||0)>=3;const checks={damage:Math.min(100,Math.round(ratio*82+(has('debuff')?8:0)+(has('aoe')&&needsAoe?8:0))),survival:Math.min(100,Math.round(ratio*72+(has('heal')?12:0)+(has('shield')?8:0)+(fireproof&&fireThreat?18:0)+Math.min(10,avg('resistance')/6))),tempo:Math.min(100,Math.round(ratio*70+Math.min(24,avg('spd')/7)+(has('control')?6:0))),sustain:has('heal')?'Adapté':has('shield')?'Partiel':'Absent',cleanse:has('cleanse')?'Présente':'Absente',fire:fireThreat?(fireproof?'Ignifuge actif':'Faible'):'Non requis'};const gaps=[];if((mission.teamSize||3)>chosen.length)gaps.push(`Composition incomplète : ${chosen.length}/${mission.teamSize} champions.`);if(needsSustain&&!has('heal'))gaps.push('Aucun soin fiable pour un combat prolongé.');if(needsCleanse&&!has('cleanse'))gaps.push('Aucune purification détectée.');if(fireThreat&&!fireproof)gaps.push('Aucun bonus Ignifuge actif.');if(needsAoe&&!has('aoe'))gaps.push('Peu de dégâts de zone pour gérer les serviteurs.');if(avg('accuracy')<25&&has('debuff'))gaps.push('Précision moyenne faible pour fiabiliser les malus.');let score=Math.round((checks.damage+checks.survival+checks.tempo)/3)-gaps.length*7;if(ratio<.7)score=Math.min(score,39);else if(ratio<.9)score=Math.min(score,54);else if(ratio<1.05)score=Math.min(score,69);if(needsSustain&&!has('heal'))score=Math.min(score,69);if(kind==='raid'&&!fireproof)score=Math.min(score,69);const verdict=score>=86?{key:'comfortable',label:'Confortable',icon:'🟢'}:score>=70?{key:'adapted',label:'Adapté',icon:'🔵'}:score>=55?{key:'risky',label:'Risqué',icon:'🟡'}:score>=40?{key:'insufficient',label:'Insuffisant',icon:'🟠'}:{key:'very-insufficient',label:'Très insuffisant',icon:'🔴'};return{power,recommended,ratio,score,verdict,checks,gaps,kind};}
