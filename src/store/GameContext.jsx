...
export function GameProvider({children}){
  ...
  /**
   * Estime les chances de victoire en jouant reellement la mission, plusieurs
   * fois, avec l'equipe et l'equipement du joueur. On remplace un nombre
   * approximatif par une reponse mesuree.
   */
  const estimerMission=(mission,members=team,tirages)=>{
    if(!mission||!members?.length)return null;
    const combattants=HEROES.map(hero=>({...hero,
      currentStars:getProgress(hero).stars,
      skillLevels:skillLevels[hero.id]||{},
      empreinteSkills:empreinteBonuses(hero,getProgress(hero).empreintes).skills,
      uniqueWeapon:getUniqueWeaponForHero(hero)}));
    const resultat=simulerMission({mission,team:members,heroes:combattants,
      getStats:hero=>totalStats(hero,equipment,getProgress(hero),inventory),
      priorites:autoSkillPriorities,tirages});
