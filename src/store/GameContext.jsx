...
import{CONTINENTS,DIFFICULTIES,STAR_MILESTONES,allMissionKeys,milestoneKey,createMission,continentClearKey,continentClearReward,continentCleared}from'../data/campaign';
import{BANNERS,DEFAULT_BANNER,bannerById,monthlyFeatured,resolveFiveStar,SUMMON_COST,PITY_THRESHOLD}from'../data/banners';
import{empreinteStatus,empreinteBonuses,BRANCHES,ETAGE_RESONANCE,RESONANCE_POINT_TIERS}from'../data/empreintes';
import{LEGENDARY_MATERIALS,CHRONICLE_REQUIREMENTS,chronicleStepStatus,chronicleStepCost,legendaryMaterialDrops,chronicleGrindLength}from'../data/legendary';
import{WORLD_BOSSES}from'../data/worldBosses';
import{load,save,day}from'../utils/storage';
import{emptyProgressionStats,mergeChampionStats}from'../utils/progressionStats'
import{canClaimAcademyLesson,canClaimAcademyFinal}from'../utils/academy'
import{canClaimAchievement,achievementGearConfigs}from'../utils/achievements'
...
import{championsDeCombat as construireChampions}from'../utils/combatChampions';
...
import{mythicRunTier,scaleMythicReward}from'../utils/mythic';
...
const freshDaily=()=>({date:day(),progress:{battle:0,skills:0,summon:0},claimed:{},bonus:false,campaignStones:0,sweeps:0});
/**
 * Balayages quotidiens. Rejouer une mission deja maitrisee pour du butin est du
 * confort, pas du contenu : on l'offre sans combat. La limite garde le rythme
 * gacha — le farm reste un choix de la journee, pas un bouton infini.
 */
export const SWEEP_DAILY_LIMIT=12;
...
export function GameProvider({children}){
  ...
  const[electedChampion,setElectedChampionState]=useState(stored.electedChampion??null);
  const[electedGuarantee,setElectedGuarantee]=useState(stored.electedGuarantee??false);
  ...
  /* « Estimer mes chances » a ete retire de l'interface : savoir a l'avance
     qu'on gagne 17 fois sur 20 vidait le combat de son enjeu. Le simulateur
     (`src/utils/simulation.js`) reste teste et disponible pour mesurer
     l'equilibrage, mais le jeu ne s'en sert plus. */
  /* ---- Défi de la semaine ------------------------------------------- */
  ...
  const progressQuest=(event,amount=1,payload={})=>{
    const update=(setter,quests)=>setter(current=>
      ({...current,progress:advanceQuestProgress(quests,current.progress,event,amount,payload)}));
    update(setDaily,QUESTS);update(setWeekly,WEEKLY_QUESTS);update(setMonthly,MONTHLY_QUESTS);
    update(setPermanentQuests,QUEST_GROUPS.progression||[]);
  };
  ...
  const claimQuest=(period,questId)=>{const quests=QUEST_GROUPS[period],quest=quests?.find(item=>item.id===questId),state=cycleState[period],setter=cycleSetter[period];if(!canClaimQuest(quest,state))return false;const reward=quest.reward||{};grantReward(reward);setter(current=>({...current,claimed:{...current.claimed,[questId]:true}}));if(period==='weekly')emitProgressEvent('weeklyQuestClaimed');return true;};
  const claimQuestChest=period=>{const quests=QUEST_GROUPS[period],state=cycleState[period],setter=cycleSetter[period],reward=FINAL_CHESTS[period];if(!canClaimChest(period,quests,state))return false;grantReward(reward);setter(current=>({...current,bonus:true}));return true;};
  ...
  /** Etat complet de l'arbre d'Empreintes d'un champion. */
  const getEmpreinteStatus=hero=>empreinteStatus(hero,getProgress(hero),getProgress(hero).empreintes);
  /** Allume un noeud. Refuse tout ce que empreinteStatus juge indisponible. */
  const lightEmpreinte=(heroId,noeudId)=>{
    ...
    if(!noeud.precedent)return{ok:false,message:'Grave d’abord l’Empreinte précédente de cette branche.'};
    if(etat.restants<=0)return{ok:false,message:'Plus aucun point d’Empreinte disponible.'};
    setChampionProgress(current=>{
      const courant=normalizeChampionProgress(hero,current[heroId]);
      if(courant.empreintes.includes(noeudId))return current;
      return{...current,[heroId]:{...courant,empreintes:[...courant.empreintes,noeudId]}};
    });
    return{ok:true,message:`${noeud.name} gravée.`,noeud};
  };
  /**
   * Efface tout l'arbre d'un champion. Gratuit et sans limite : essayer une
   * autre orientation ne doit rien couter dans un jeu entre amis, et les points
   * eux-memes restent gagnes par l'Ascension.
   */
  const resetEmpreintes=heroId=>{
    const hero=HEROES.find(entry=>entry.id===heroId);
    if(!hero)return{ok:false,message:'Champion introuvable.'};
    setChampionProgress(current=>({...current,[heroId]:{...normalizeChampionProgress(hero,current[heroId]),empreintes:[]}}));
    return{ok:true,message:'Empreintes effacées.'};
  };
  ...
  /** Le 5★ nomme sur le Vœu d'Azerune. Changer d'elu desarme la garantie. */
  const setElectedChampion=id=>{
    const hero=HEROES.find(entry=>entry.id===id&&entry.rarity===5);
    if(id!==null&&!hero)return{ok:false,message:'Seul un champion 5★ peut être nommé.'};
    if(electedChampion===id)return{ok:true,message:'Élu inchangé.'};
    setElectedChampionState(id);setElectedGuarantee(false);
    return{ok:true,message:id?`${hero.name} devient votre élu.`:'Élu retiré.'};
  };
  const monthlyBanner=monthlyFeatured(new Date(),HEROES);
  const summonMany=(count,currency,bannerId=DEFAULT_BANNER)=>{
    const banner=bannerById(bannerId);
    if(currency==='stone'&&!banner.acceptsStones)return null;
    const cost=currency==='stone'?count:(count===10?SUMMON_COST.multi:count*SUMMON_COST.single);
    ...
    let guaranteeDraft=electedGuarantee;
    const results=[];
    let obtainedFourPlus=false,bloodGained=0;
    for(let index=0;index<count;index+=1){
      const pityGuaranteed=pityDraft>=PITY_THRESHOLD-1;
      const x10Guarantee=count===10&&index===9&&!obtainedFourPlus&&!pityGuaranteed;
      let hero=pickHero(pityGuaranteed,x10Guarantee?4:3);
      // Le portail ne change pas la rarete tiree — seulement LEQUEL des 5★ sort.
      // C'est ce qui garde l'economie identique d'un portail a l'autre.
      let onTarget=false;
      if(hero.rarity===5){
        const cible=resolveFiveStar({bannerId:banner.id,electedId:electedChampion,featured:monthlyBanner,
          guaranteed:guaranteeDraft,roll:Math.random(),
          pick:list=>list[Math.floor(Math.random()*list.length)],heroes:HEROES});
        guaranteeDraft=cible.guaranteed;onTarget=cible.onTarget;
        hero=HEROES.find(entry=>entry.id===cible.id)||hero;
      }
      ...
      results.push({...hero,duplicate,summonResult:outcome.result,progress:outcome.progress,bloodFragments:outcome.bloodFragments||0,pityGuaranteed,x10Guaranteed:x10Guarantee,bannerId:banner.id,onTarget});
    }
    setOwned(ownedDraft);setChampionProgress(progressDraft);setPityCounter(pityDraft);setElectedGuarantee(guaranteeDraft);if(bloodGained)setBloodFragments(value=>value+bloodGained);
    ...
    emitProgressEvent('heroSummoned',{amount:count});setProgressionStats(current=>({...current,lifetime:{...current.lifetime,summons:{...current.lifetime.summons,total:current.lifetime.summons.total+count,multi10:current.lifetime.summons.multi10+(count===10?1:0),newHeroes:current.lifetime.summons.newHeroes+results.filter(x=>!x.duplicate).length,duplicates:current.lifetime.summons.duplicates+results.filter(x=>x.duplicate).length,rarity4:current.lifetime.summons.rarity4+results.filter(x=>x.rarity===4).length,rarity5:current.lifetime.summons.rarity5+results.filter(x=>x.rarity===5).length,pity:current.lifetime.summons.pity+results.filter(x=>x.pityGuaranteed).length,gemsSpent:current.lifetime.summons.gemsSpent+(currency==='gems'?cost:0),stonesSpent:current.lifetime.summons.stonesSpent+(currency==='stone'?cost:0)}}}));
    grantSummonerXp(count*8);
    return results;
  };
  const summon=()=>summonMany(1,'gems',DEFAULT_BANNER)?.[0]||null;
  ...
  const chronicleContext=()=>({materials:legendaryChronicles.materials,relics:legendaryChronicles.relics,
    kills:legendaryChronicles.kills,stats:progressionStats,
    resources:{mythicEssence:ascensionEssences.mythic,forgeEssence}});
  const getChronicleStatus=(weaponId,step)=>chronicleStepStatus(weaponId,step,chronicleContext());
  /**
   * Valide une etape. C'etait un bouton libre : le joueur cliquait trois fois
   * et forgeait l'arme, les 80 Lingots volcaniques n'etant comptes par rien.
   * L'etape ne passe desormais que si son exigence est remplie, et les Vestiges
   * sont consommes au passage.
   */
  const advanceChronicle=weaponId=>{
    const steps=CHRONICLE_STEPS[weaponId];
    if(!steps||!legendaryChronicles.active[weaponId])return{ok:false,message:'Chronique inactive.'};
    const step=Math.max(0,Math.min(steps.length-1,legendaryChronicles.active[weaponId].step||0));
    if(step>=steps.length-1)return{ok:false,message:'Dernière étape : forge l’arme Unique.'};
    const statut=getChronicleStatus(weaponId,step);
    if(!statut.ready){
      const manque=statut.lines.filter(l=>l.have<l.need)
        .map(l=>`${l.label} ${l.have.toLocaleString('fr-FR')}/${l.need.toLocaleString('fr-FR')}`).join(' · ');
      return{ok:false,message:`Étape incomplète : ${manque}`};
    }
    const cout=chronicleStepCost(weaponId,step);
    setLegendaryChronicles(v=>{
      const materials={...v.materials};
      Object.entries(cout).forEach(([id,montant])=>{materials[id]=Math.max(0,(materials[id]||0)-montant)});
      return{...v,materials,active:{...v.active,[weaponId]:{...(v.active[weaponId]||{}),step:step+1}}};
    });
    return{ok:true,message:`${steps[step][0]} — étape franchie.`};
  };
  ...
  /** Vestiges tombes a la fin d'une activite, pour les Chroniques actives. */
  const rollLegendaryMaterials=(activity,level)=>{
    const gains=legendaryMaterialDrops(activity,level,legendaryChronicles.active);
    const entrees=Object.entries(gains);
    if(!entrees.length)return null;
    setLegendaryChronicles(v=>{
      const materials={...v.materials};
      entrees.forEach(([id,montant])=>{materials[id]=(materials[id]||0)+montant});
      return{...v,materials};
    });
    return entrees.map(([id,montant])=>({id,amount:montant,...LEGENDARY_MATERIALS[id]}));
  };
  /** Enregistre une victoire que des Chroniques attendent. */
  const recordChronicleKill=id=>{if(!id)return;setLegendaryChronicles(v=>({...v,kills:{...v.kills,[id]:(v.kills?.[id]||0)+1}}))};
  ...
  // Les champions tels qu'ils ENTRENT en combat. Le constructeur vit dans
  // src/utils/combatChampions.js : le vrai combat et tout ce qui voudra
  // rejouer une mission doivent partir exactement du meme etat.
  const championsDeCombat=()=>construireChampions(HEROES,
    {getProgress,skillLevels,uniqueWeaponFor:getUniqueWeaponForHero});
// Taux revus : voir raidRelicChance pour le raisonnement. Les anciens exigeaient
  // 462 a 1 386 passages pour une chance sur deux, et 2 022 pour les deux Liens
  // du Prince-Tempete reunis. Une relique reste tres rare — moins d'une chance
  // sur deux cents — mais l'aventure devient vivable.
  const rollLegendaryMythic=mission=>{const l=mission.mythicLevel,boss=mission.bossInfo?.name||'';let candidates=[];if(l===20)candidates.push(['storm-left',.004],['ash-shard',.0035],['ancient-tear',.0035]);if(l===30)candidates.push(['storm-right',.005]);if(l>=25&&l<=29)candidates.push(['fallen-plume',.002]);if(l===30)candidates.push(['fallen-plume',.005]);if(l>=18&&l<=20)candidates.push(['first-plague',.0025]);if(l>=21&&l<=29)candidates.push(['first-plague',.004]);if(l===30&&boss.includes('Astreon'))candidates.push(['eclipse-string',.006]);for(const [id,chance] of candidates)if(Math.random()<chance){const r=grantRelic(id,`Mythic+ ${l}`);if(r.ok)return r}return null;};
  ...
  const claimAchievement=id=>{const achievementState={campaign,expeditionProgress,raidProgress,mythicProgress,owned,championProgress,forgeHistory,inventory,history,legendaryChronicles,progressionStats};const controle=canClaimAchievement(id,achievementState,achievementClaims);if(!controle.ok)return controle;const{achievement,reward}=controle;if(reward.gold)setGold(value=>value+reward.gold);if(reward.gems)setGems(value=>value+reward.gems);if(reward.essence)setForgeEssence(value=>value+reward.essence);const gear=achievementGearConfigs(reward).map(generateAchievementItem);if(gear.length)setInventory(current=>[...gear,...current].slice(0,INVENTORY_LIMIT));setAchievementClaims(current=>({...current,[id]:Date.now()}));return{ok:true,message:`Récompense de ${achievement.name} reçue.`};};
  const finishMythicMission=(mission,battle)=>{const uniqueRelic=rollLegendaryMythic(mission);const level=mission.mythicLevel,tier=mythicRunTier(battle?.mythicState),reward=scaleMythicReward(mission.reward,tier),first=!mythicProgress.claimed?.[level];
    // Les Vestiges tombent a CHAQUE passage, pas au seul premier : il n'existe
    // que trente niveaux, et une etape en demande vingt a soixante. Les gager
    // sur le premier clear rendrait la quete impossible a terminer.
    const legendaryMaterials=rollLegendaryMaterials('mythic',level);
    if(level>=30){recordChronicleKill('mythic-30');
      if(String(mission.bossInfo?.name||'').includes('Astreon'))recordChronicleKill('mythic-30-astreon');}
    if(!first)return{mythic:true,first:false,gold:0,gems:0,essence:0,uniqueRelic,legendaryMaterials,mythicTier:tier};if(reward.gold)setGold(v=>v+reward.gold);if(reward.gems)setGems(v=>v+reward.gems);if(reward.stones)setHearthstones(v=>v+reward.stones);if(reward.essence)setForgeEssence(v=>v+reward.essence);if(reward.tomes)setMasteryTomes(v=>v+reward.tomes);if(reward.souls)setUniversalSoul5(v=>v+reward.souls);const loot=reward.gear?generateMythicItem({level,source:`Mythic+ · ${mission.mythicSeason} · Niveau ${level}`}):null;if(loot)setInventory(v=>[loot,...v].slice(0,INVENTORY_LIMIT));setMythicProgress(current=>({...current,completed:Math.max(current.completed||0,level),claimed:{...(current.claimed||{}),[level]:true}}));emitProgressEvent('mythicLevelReached',{amount:1,value:level});return{mythic:true,first:true,gold:reward.gold||0,gems:reward.gems||0,essence:reward.essence||0,stones:reward.stones||0,tomes:reward.tomes||0,souls:reward.souls||0,loot,uniqueRelic,legendaryMaterials,mythicTier:tier};};
  ...
  /**
   * Victoire sur un Adversaire legendaire.
   *
   * Elle ne renvoyait rien du tout : l'ecran de combat retournait `null` pour
   * un boss de monde, si bien qu'on pouvait vaincre Aeralion indefiniment sans
   * qu'aucune Chronique n'avance. C'est la seule activite du jeu dont la
   * victoire ne comptait nulle part.
   */
  const finishWorldBossMission=mission=>{
    const boss=WORLD_BOSSES[mission.worldBossId];
    const reward={worldBoss:true,gold:0,gems:0,bossName:boss?.name||mission.name,
      legendaryMaterials:rollLegendaryMaterials('worldboss',1)};
    if(boss){recordChronicleKill(`worldboss-${boss.id}`);reward.chronicleKill=boss.name;}
    emitProgressEvent('bossDefeated');
    return reward;
  };
  ...
  /**
   * Recolte d'une mission deja terminee au meme score. Partagee par le combat
   * rejoue et par le Balayage : les deux doivent donner exactement la meme
   * chose, sinon balayer devient un choix d'optimisation au lieu d'un confort.
   */
  const recolteDeFarm=(mission,previous,xpResult)=>{
    const farmGold=campaignFarmGold(mission),lootRoll=rollCampaignLoot(mission,false),campaignStone=rollCampaignStone(mission);
    setGold(value=>value+farmGold);
    return{gold:farmGold,gems:0,stones:0,improved:false,farm:true,previousStars:previous,
      loot:lootRoll.item,lootChance:lootRoll.lootChance,campaignStone,
      championXp:xpResult.xp,xpFactor:xpResult.factor,xpFarm:true};
  };
  /** Balayages restants aujourd'hui. */
  const sweepsLeft=()=>Math.max(0,SWEEP_DAILY_LIMIT-Number(daily.sweeps||0));
  /** Une mission est balayable quand elle est deja maitrisee a 3 etoiles. */
  const canSweep=mission=>Boolean(mission)&&!mission.raid&&!mission.mythic&&!mission.expedition&&!mission.worldBoss
    &&(campaign.scores[mission.key]||0)>=3&&sweepsLeft()>0;
  /**
   * Balaye une mission maitrisee : meme recolte qu'un combat rejoue, sans le
   * combat. L'XP passe par le meme calcul, donc la puissance de l'equipe
   * compte toujours.
   */
  const sweepCampaignMission=(mission,members=team)=>{
    if(!canSweep(mission))return null;
    ...
    const xpResult=campaignXp(campaignBaseXp(mission,false),teamValue,mission.recommended);
    if(xpResult.xp)grantXp(members,xpResult.xp);
    setDaily(current=>({...current,sweeps:Number(current.sweeps||0)+1}));
    return{...recolteDeFarm(mission,previous,xpResult),sweep:true,sweepsLeft:sweepsLeft()-1};
  };
  const finishCampaignMission=(mission,stars,members=team)=>{
    ...
    if(xpResult.xp)grantXp(members,xpResult.xp);
    if(!improved)return recolteDeFarm(mission,previous,xpResult);
    ...
    // Palier de continent : verse au premier nettoyage complet des sept
    // missions, une seule fois par difficulte. On calcule sur les scores
    // resultants et non sur `campaign.scores`, qui n'a pas encore ete mis a
    // jour par ce setState.
    const scoresApres={...campaign.scores,[mission.key]:stars};
    const continent=CONTINENTS.find(entry=>entry.id===mission.continentId);
    const clearKey=continent?continentClearKey(mission.difficultyId,continent.id):null;
    const paliersDeja=campaign.clearedContinents||{};
    const palier=continent&&!paliersDeja[clearKey]&&continentCleared(scoresApres,mission.difficultyId,continent)
      ?continentClearReward(CONTINENTS.indexOf(continent),mission.difficultyId):null;
    if(palier){setGems(value=>value+palier.gems);setGold(value=>value+palier.gold);if(palier.stones)setHearthstones(value=>value+palier.stones);}
    setCampaign(current=>({...current,scores:{...current.scores,[mission.key]:stars},
      clearedContinents:palier?{...(current.clearedContinents||{}),[clearKey]:Date.now()}:(current.clearedContinents||{})}));
    emitProgressEvent('campaignStarsEarned',{amount:Math.max(0,stars-previous)});
