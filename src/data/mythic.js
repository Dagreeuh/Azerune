...
// Montee en puissance des ennemis Mythic+.
//
// L'Attaque suivait 1,035 par palier quand la puissance des joueurs suivait
// 1,042 : l'ecart se creusait a chaque niveau, et surtout la Defense alliee
// croissait plus vite encore. Avec la mitigation `100/(100+DEF*3)`, un joueur
...
export function createMythicMission(level){level=Math.max(1,Math.min(30,Number(level)||1));const season=mythicSeason(),affixCount=level<=4?0:level<=9?1:level<=19?2:3,affixIds=season.affixes.slice(0,affixCount),waves=[1,2,3,4].map(index=>wave(level,index,season)),recommended=Math.round(12650+28*level*level);return{key:`mythic:${season.key}:${level}`,name:`Mythic+ ${level}`,icon:'🗝️',mythic:true,teamSize:4,mythicLevel:level,mythicSeason:season.key,waves,enemies:waves[0],affixIds,affixes:affixIds.map(id=>MYTHIC_AFFIXES[id]).filter(Boolean),bossInfo:waves[3][0].mythicBossInfo||{name:waves[3][0].name,icon:waves[3][0].icon,element:waves[3][0].element,passive:'Boss de palier',spells:[['Pouvoir majeur','Attaque de zone toutes les trois actions.']],tips:['Éliminer les serviteurs avant le boss.','Conserver soins et contrôles pour la quatrième vague.']},recommended,turnBudget:mythicTurnBudget(waves,recommended),reward:rewards(level),scale:1};}
