...
function SkillTooltip({actor,skill,index,relation,unlocked,onGuide,defense=0}){
 ...
 <section><small>STATISTIQUES CLÉS</small><b>{info.scaling.join(' · ')}</b></section><section><small>CONSEIL</small><p>{info.gear}</p></section><section className={`tooltip-affinity affinity-${relation.key}`}><small>AFFINITÉ</small><b>{relation.icon} {relation.label}</b><p>{relation.detail}</p><em>{detailAffinite(affinity('Feu','Nature',battle?.difficulte))} — inefficace ×{affinity('Nature','Feu',battle?.difficulte).damage.toFixed(2).replace('.',',')}</em></section><button type="button" className="tooltip-guide-button" onClick={event=>{event.stopPropagation();onGuide?.(actor)}}>ⓘ Guide du champion</button></>}</div>}
