...
function Unit({unit,active,selected,automatic,onClick,events=[],enemies=[],allies=[],vfxEnabled=true}){const shieldMax=Math.max(unit.maxShield||0,unit.shield||0,1),element=elementMeta(unit.element),ghoulTurns=Math.max(0,unit.mechanic?.ghoulTurns||0),ghoulDamage=Math.max(0,unit.mechanic?.ghoulDamage||Math.round((unit.atk||0)*.28)),livingEnemies=(enemies||[]).filter(enemy=>!enemy.dead),isKaelen=unit.id===3||unit.skills?.some(skill=>skill.effect==='huntMark'),kaelenTarget=isKaelen?livingEnemies.find(enemy=>enemy.id===unit.mechanic?.targetId&&enemy.debuffs?.hunt?.source===unit.id):null,kaelenTurns=kaelenTarget?.debuffs?.hunt?.turns||0,isVaeloria=unit.id===7||unit.skills?.some(skill=>skill.effect==='bladeDanceStorm'),isBrom=unit.id===8||unit.skills?.some(skill=>skill.effect==='impactQuake'),isVelomoteur=unit.id===9,isDagcat=unit.id===10,isMobeen=unit.id===11,isLelianna=unit.id===12,atonements=isLelianna?allies.filter(ally=>ally.buffs?.atonement?.source===unit.id):[],isSaylich=unit.id===13,isBrilith=unit.id===14,isHicho=unit.id===15,isHisteria=unit.id===17||unit.skills?.some(skill=>skill.effect==='rapture'),histeriaAfflictions=isHisteria?livingEnemies.map(enemy=>{const keys=['agony','corruption','poison','burn','bleed'],active=keys.filter(key=>enemy.debuffs?.[key]),agonyStacks=Math.max(0,enemy.debuffs?.agony?.stacks||0),ready=active.length>=2||agonyStacks>=3;return{enemy,active,agonyStacks,ready,score:(ready?100:0)+active.length*10+agonyStacks}}).filter(value=>value.active.length).sort((a,b)=>b.score-a.score)[0]:null,histeriaLabel=histeriaAfflictions?(histeriaAfflictions.ready?`EXTASE PRÊTE · ${histeriaAfflictions.enemy.name}`:`${histeriaAfflictions.active.length} affliction${histeriaAfflictions.active.length>1?'s':''} · ${histeriaAfflictions.enemy.name}${histeriaAfflictions.agonyStacks?` · Agonie ${histeriaAfflictions.agonyStacks}`:''}`):'Aucune affliction',isMalvek=unit.id===22||unit.skills?.some(skill=>skill.effect==='virulentStrike'),isSivrane=unit.id===33||unit.skills?.some(skill=>skill.effect==='frostShatter'),
  // Virulence et Givre ne vivent pas sur le champion mais sur SA CIBLE : ce sont
  // des cumuls de malus. Leur barre lisait le compteur personnel du champion,
  // reste a zero pour toujours. On lit desormais l'ennemi le plus charge.
  enemyStacks=cle=>livingEnemies.map(enemy=>({enemy,stacks:Math.max(0,enemy.debuffs?.[cle]?.stacks||0)})).filter(entry=>entry.stacks>0).sort((a,b)=>b.stacks-a.stacks)[0]||null,
  malvekStacks=isMalvek?enemyStacks('virulence'):null,sivraneStacks=isSivrane?enemyStacks('frost'):null,
  isSylven=unit.id===19,isNerissa=unit.id===21,isAurelis=unit.id===23,isVexil=unit.id===24,isIgnovar=unit.id===26,isElowen=unit.id===27,isSeraphiel=unit.id===28,isThorgar=unit.id===1||unit.skills?.some(skill=>skill.effect==='guardianLink'),thorgarTarget=isThorgar?allies.find(ally=>ally.id===unit.mechanic?.targetId&&ally.buffs?.guardianLink?.source===unit.id):null,thorgarTurns=thorgarTarget?.buffs?.guardianLink?.turns||0,isKorga=unit.name==='Korga'||unit.skills?.some(skill=>skill.effect==='shieldExecute'),korgaExposed=isKorga?livingEnemies.find(enemy=>enemy.debuffs?.exposed):null,korgaBroken=isKorga?livingEnemies.find(enemy=>enemy.shieldBroken||((enemy.maxShield||0)>0&&(enemy.shield||0)<=0)):null,korgaTarget=korgaExposed||korgaBroken,korgaReady=Boolean(korgaTarget),isCaelion=unit.id===30||unit.skills?.some(skill=>skill.effect==='timeAnchor'),caelionTarget=isCaelion?allies.find(ally=>ally.id===unit.mechanic?.targetId):null,isMorghast=unit.id===29||unit.skills?.some(skill=>String(skill.effect||'').startsWith('alchemy')),morghastReaction=isMorghast?livingEnemies.map(enemy=>{const poison=Boolean(enemy.debuffs?.poison),burn=Boolean(enemy.debuffs?.burn),bleed=Boolean(enemy.debuffs?.bleed),count=[poison,burn,bleed].filter(Boolean).length,label=count===3?'Catalyse parfaite':poison&&bleed?'Hémotoxique':poison&&burn?'Caustique':burn&&bleed?'Thermique':null;return label?{enemy,label,count}:null}).filter(Boolean).sort((a,b)=>b.count-a.count)[0]:null;return <div className={`unit-summon-stack ${ghoulTurns?'has-summon':''}`}>{ghoulTurns>0&&<aside className="ghoul-summon-card"><span>💀</span><div><b>Goule de Nashoba</b><small>{ghoulTurns} attaque{ghoulTurns>1?'s':''} restante{ghoulTurns>1?'s':''} · {ghoulDamage} dégâts prévus</small></div></aside>}<button disabled={unit.dead} onClick={onClick} className={`unit battle-unit compact-unit ${active?'active':''} ${selected?'target-selected':''} ${automatic?'auto-target':''} ${events.some(event=>event.critical)?'impact-shake':''} ${events.length?'vfx-hit':''} ${unit.shield>0?'shielded':''}`} style={{'--element-color':element.color}}>{active&&<span className="turn-label">TOUR</span>}<SpellVfx events={events} enabled={vfxEnabled}/><EventFloat events={events}/><div className="unit-identity"><b className="emoji">{unit.dead?'☠️':unit.icon}</b><div><strong>{unit.name}</strong><span className="mini-element">{element.icon} {element.name}</span></div></div><div className="unit-bars"><div><span>PV {Math.ceil(unit.hp)}/{unit.maxHp}</span><Bar v={unit.hp} max={unit.maxHp}/></div>{unit.shield>0&&<div className="mini-shield"><span>🔷 {unit.shield}</span><Bar v={unit.shield} max={shieldMax} color="#38bdf8"/></div>}<div><span>Jauge {Math.round(unit.atb)} % · VIT {unit.currentSpd}</span><Bar v={unit.atb} max={100} color="#22d3ee"/></div></div><EffectList unit={unit}/>{unit.side==='ally'&&(isKaelen?<div className={`champion-resource kaelen-hunt ${kaelenTarget?'active':''}`}><span>{kaelenTarget?'🎯 PROIE MARQUÉE':'🎯 TRAQUE'}</span><b>{kaelenTarget?`${kaelenTarget.name} · ${kaelenTurns} tour${kaelenTurns>1?'s':''}`:'Aucune proie'}</b></div>:isVaeloria?<div className={`champion-resource vaeloria-dance ${(unit.mechanic?.value||0)>0?'active':''}`}><span>🗡️ DANSE DES LAMES</span><b>{unit.mechanic?.value||0}/3 étapes</b><small>{unit.skills?.map((skill,index)=>{const used=(unit.mechanic?.danceSteps||[]).includes(index+1);return `${used?'✓':'○'} ${index+1} ${skill.name}`}).join(' · ')}</small></div>:isVelomoteur?<div className={`champion-resource velomoteur-power ${(unit.mechanic?.value||0)>=5?'active':''}`}><span>⚜️ PUISSANCE SACRÉE</span><b>{unit.mechanic?.value||0}/5{(unit.mechanic?.value||0)>=5?' · Verdict prêt':''}</b></div>:isDagcat?<div className={`champion-resource dagcat-combo ${(unit.mechanic?.value||0)>=3?'active':''}`}><span>🐾 COMBO FAROUCHE</span><b>{unit.mechanic?.value||0}/5</b></div>:isMobeen?<div className={`champion-resource mobeen-combo ${unit.mechanic?.active?'active':''}`}><span>{unit.mechanic?.active?'🥷 OUVERTURE PRÊTE':'🗡️ COMBO FURTIF'}</span><b>{unit.mechanic?.value||0}/5</b></div>:isLelianna?<div className={`champion-resource lelianna-atonement ${atonements.length?'active':''}`}><span>🕊️ EXPIATIONS</span><b>{atonements.length} allié{atonements.length>1?'s':''} lié{atonements.length>1?'s':''}</b></div>:isSaylich?<div className={`champion-resource saylich-aim ${unit.mechanic?.active?'active':''}`}><span>{unit.mechanic?.active?'🎯 TIR PRÊT':'🎯 VISÉE'}</span><b>{unit.mechanic?.active?'Critique garanti':`${unit.mechanic?.value||0}/3`}</b></div>:isHisteria?<div className={`champion-resource histeria-afflictions ${histeriaAfflictions?.ready?'active':''}`}><span>{histeriaAfflictions?.ready?'🕯️ EXTASE PRÊTE':'🕯️ AFFLICTIONS'}</span><b>{histeriaLabel}</b></div>:isHicho?<div className={`champion-resource final-resource hicho-totem ${unit.mechanic?.active?'active':''}`}><span>🗿 TOTEM</span><b>{unit.mechanic?.active?`${unit.mechanic.value} tour(s)`:'Inactif'}</b></div>:isSylven?<div className={`champion-resource final-resource sylven-seeds ${(unit.mechanic?.value||0)>0?'active':''}`}><span>🌱 GRAINES</span><b>{unit.mechanic?.value||0}/3</b></div>:isNerissa?<div className={`champion-resource final-resource nerissa-reflux ${(unit.mechanic?.value||0)>0?'active':''}`}><span>🌊 REFLUX</span><b>{unit.mechanic?.value||0}/60</b></div>:isAurelis?<div className={`champion-resource final-resource aurelis-aegis ${unit.mechanic?.active?'active':''}`}><span>🔷 ÉGIDE</span><b>{unit.mechanic?.active?(unit.mechanic.targetId?'Réservée':'Prête'):'Consommée'}</b></div>:isVexil?<div className={`champion-resource final-resource vexil-instability ${(unit.mechanic?.value||0)>=5?'danger active':(unit.mechanic?.value||0)>=4?'warning active':''}`}><span>{(unit.mechanic?.value||0)>=5?'☢️':(unit.mechanic?.value||0)>=4?'⚠️':'🌀'} INSTABILITÉ</span><b>{unit.mechanic?.value||0}/5 · {(unit.mechanic?.value||0)>=5?'Critique · 12 % puis retour à 3':(unit.mechanic?.value||0)>=4?'Instable · 6 % au prochain sort':'Stable'}</b></div>:isIgnovar?<div className={`champion-resource final-resource ignovar-embers ${(unit.mechanic?.value||0)>=3?'active':''}`}><span>🔥 BRAISES</span><b>{unit.mechanic?.value||0}/5 · {livingEnemies.filter(enemy=>enemy.debuffs?.burn).length} brûlé(s)</b></div>:isElowen?<div className={`champion-resource final-resource elowen-garden ${unit.mechanic?.active?'active':''}`}><span>🌳 JARDIN</span><b>{unit.mechanic?.active?`${unit.mechanic.value} tour(s)`:'Inactif'}</b></div>:isSeraphiel?<div className={`champion-resource final-resource seraphiel-condemn ${(unit.mechanic?.value||0)>=3?'active':''}`}><span>⚖️ CONDAMNATION</span><b>{unit.mechanic?.value||0}/6</b></div>:isBrilith?<div className={`champion-resource brilith-arcane ${(unit.mechanic?.value||0)>=4?'active':''}`}><span>🔮 CHARGES ARCANIQUES</span><b>{unit.mechanic?.value||0}/4{(unit.mechanic?.value||0)>=4?' · Barrage optimal':''}</b></div>:isBrom?<div className={`champion-resource brom-impact ${(unit.mechanic?.value||0)>=3?'active':''}`}><span>{(unit.mechanic?.value||0)>=3?'🌋 SÉISME PRÊT':'🔨 IMPACT'}</span><b>{unit.mechanic?.value||0}/3</b></div>:isThorgar?<div className={`champion-resource thorgar-link ${thorgarTarget?'active':''}`}><span>{thorgarTarget?'🛡️ SERMENT ACTIF':'🛡️ LIEN RUNIQUE'}</span><b>{thorgarTarget?`${thorgarTarget.name} · ${thorgarTurns} tour${thorgarTurns>1?'s':''}`:'Aucun allié lié'}</b></div>:isCaelion?<div className={`champion-resource caelion-anchor ${unit.mechanic?.active?'active':''}`}><span>{unit.mechanic?.anchorSpent?'⏳ RETOUR PRÊT':'⏳ ANCRAGE'}</span><b>{caelionTarget?`${caelionTarget.name} · ${unit.mechanic?.anchorSpent?'Restaurable':'En attente'}`:'Aucun allié ancré'}</b></div>:isKorga?<div className={`champion-resource korga-fracture ${korgaReady?'active':''}`}><span>{korgaReady?'⚔️ EXÉCUTION PRÊTE':'💥 FRACTURE'}</span><b>{korgaTarget?korgaTarget.name:'Aucune cible exposée'}</b></div>:isMorghast?<div className={`champion-resource morghast-reaction ${morghastReaction?'active':''}`}><span>🧪 Réactions alchimiques</span><b>{morghastReaction?`${morghastReaction.label} · ${morghastReaction.enemy.name}`:'Combine 2 afflictions'}</b></div>:isMalvek?<div className={`champion-resource malvek-virulence ${(malvekStacks?.stacks||0)>=3?'active':''}`}><span>☠️ Virulence sur la cible</span><b>{malvekStacks?`${malvekStacks.stacks} cumul${malvekStacks.stacks>1?'s':''} · ${malvekStacks.enemy.name}`:'Aucune cible infectée'}</b></div>:isSivrane?<div className={`champion-resource sivrane-frost ${(sivraneStacks?.stacks||0)>=3?'active':''}`}><span>❄️ Givre sur la cible</span><b>{sivraneStacks?`${sivraneStacks.stacks}/5 · ${sivraneStacks.enemy.name}${sivraneStacks.stacks>=3?' · BRISURE PRÊTE':''}`:'Aucune cible givrée'}</b></div>:championIdentity(unit).resource==='Aucune'?null:<div className={`champion-resource ${unit.mechanic?.active?'active':''}`}><span>{championIdentity(unit).icon} {championIdentity(unit).resource}</span><b>{unit.mechanic?.mode?(unit.mechanic.mode==='high'?'MARÉE HAUTE':'MARÉE BASSE'):`${unit.mechanic?.value||0}${unit.mechanic?.max?`/${unit.mechanic.max}`:''}`}</b></div>)}</button></div>}
function SkillTooltip({actor,skill,index,relation,unlocked,onGuide,defense=0}){
 ...
 <section><small>STATISTIQUES CLÉS</small><b>{info.scaling.join(' · ')}</b></section><section><small>CONSEIL</small><p>{info.gear}</p></section><section className={`tooltip-affinity affinity-${relation.key}`}><small>AFFINITÉ</small><b>{relation.icon} {relation.label}</b><p>{relation.detail}</p><em>Efficace ×1,30 · Inefficace ×0,75</em></section><button type="button" className="tooltip-guide-button" onClick={event=>{event.stopPropagation();onGuide?.(actor)}}>ⓘ Guide du champion</button></>}</div>}
...
export default function BattlePage({setPage}){
 ...
 return <section className={`battle-screen-compact ${battle.autoMode?'auto-mode-active':''} ${vfxEnabled?'':'no-vfx'} ${battle.allies.length>=4?'battle-format-4v4':''}`}><button type="button" className={`battle-auto-toggle ${battle.autoMode?'active':''}`} onClick={toggleAuto} aria-pressed={Boolean(battle.autoMode)}>{battle.autoMode?'⏸ AUTO ACTIF':'▶ AUTO'}</button>{battle.regle&&<span className="battle-regle" title={battle.regle.summary}>{battle.regle.icon} {battle.regle.name}</span>}<button type="button" className="battle-speed-toggle" onClick={cycleSpeed} title="Vitesse du combat automatique">⏩ x{speed}</button><button type="button" className={`battle-vfx-toggle ${vfxEnabled?'active':''}`} onClick={toggleVfx} aria-pressed={vfxEnabled} title="Effets visuels des sorts">{vfxEnabled?'✨ EFFETS':'✨ EFFETS COUPÉS'}</button>{battle.mythic&&<div className="mythic-wave-bar"><b>🗝️ Mythic+ {mission.mythicLevel}</b><span>Vague {battle.wave}/{battle.totalWaves}</span><span className={`mythic-sand ${mythicCollapsed(battle.mythicState)?'collapsed':mythicSandLeft(battle.mythicState)<=15?'low':''}`}>{mythicCollapsed(battle.mythicState)?`💥 Effondrement +${mythicOvertime(battle.mythicState)}`:`⌛ ${mythicSandLeft(battle.mythicState)} tours`}</span><em>{mission.affixes.length?mission.affixes.map(a=>`${a.icon} ${a.name}`).join(' · '):'Aucun affixe'}</em></div>}
  ...
  <button type="button" className="affinity-fab" aria-label="Afficher les forces et faiblesses" aria-expanded={affinityOpen} onClick={()=>setAffinityOpen(true)}>⚖️</button>{affinityOpen&&<div className="affinity-modal-backdrop" role="presentation" onClick={()=>setAffinityOpen(false)}><aside className="affinity-popover affinity-popover-safe" role="dialog" aria-modal="true" aria-labelledby="affinity-title" onClick={event=>event.stopPropagation()}><div className="affinity-popover-heading"><h3 id="affinity-title">FORCES ET FAIBLESSES</h3><button type="button" onClick={()=>setAffinityOpen(false)} aria-label="Fermer">✕</button></div><div className="affinity-cycle"><span>🔥 Feu</span><b>›</b><span>🌿 Nature</span><b>›</b><span>💧 Eau</span><b>›</b><span>🔥 Feu</span></div><div className="affinity-cycle"><span>🔮 Arcane</span><b>›</b><span>🌑 Ombre</span><b>›</b><span>✨ Lumière</span><b>›</b><span>🔮 Arcane</span></div><div className="affinity-explanations"><small><b className="effective-text">▲ Efficace</b><span>Dégâts ×1,30 · Effets +15 %</span></small><small><b>● Neutre</b><span>Dégâts ×1,00</span></small><small><b className="weak-text">▼ Inefficace</b><span>Dégâts ×0,75 · Effets -15 %</span></small></div></aside></div>}
 </section>;
}
