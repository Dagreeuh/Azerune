...
export function castSkill(battle,index,targetId){
  ...
  if(e==='mistStrike'){const blesse=allies.filter(x=>!x.dead).sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp)[0];if(blesse)heal(blesse,blesse.maxHp*(resonanceIV?.10:.08)*(1+mastery.power));}
  ...
  if(e==='breathOfEons'){allies.filter(x=>!x.dead).forEach(x=>{x.buffs.damageUp={turns:2+mastery.duration,power:(resonanceIV?.20:.15)+(mastery.power||0),source:actor.id,label:'Souffle des éons'};x.atb=Math.min(99,(x.atb||0)+(resonanceIV?18:12));});logs.push('Souffle des éons : toute l’équipe frappe plus fort et gagne de la jauge.');}
  ...
  if(e==='atonementStrike'||e==='atonementPenance'){allies.filter(x=>!x.dead&&x.buffs.atonement?.source===actor.id).forEach(x=>heal(x,damageTotal*(resonanceIV?.40:.35)/COMBAT_TEMPO));m.value=allies.filter(x=>x.buffs.atonement?.source===actor.id).length;}
