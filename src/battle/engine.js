...
export function castSkill(battle,index,targetId){
  ...
  if(e==='prescienceStrike'){const porteur=allies.filter(x=>!x.dead&&x.id!==actor.id).sort((a,b)=>b.atk-a.atk)[0];if(porteur){porteur.buffs.damageUp={turns:2+mastery.duration,power:resonanceIV?.18:.14,source:actor.id};logs.push(`Prescience sur ${porteur.name} : dégâts augmentés.`);}}
  if(e==='ebonMight'){chosen.buffs.damageUp={turns:3+mastery.duration,power:(resonanceIV?.36:.30)+(mastery.power||0),source:actor.id};logs.push(`Puissance d’ébène : ${chosen.name} inflige ${Math.round(chosen.buffs.damageUp.power*100)} % de dégâts supplémentaires.`);}
  if(e==='breathOfEons'){allies.filter(x=>!x.dead).forEach(x=>{x.buffs.damageUp={turns:2+mastery.duration,power:(resonanceIV?.20:.15)+(mastery.power||0),source:actor.id};x.atb=Math.min(99,(x.atb||0)+(resonanceIV?18:12));});logs.push('Souffle des éons : toute l’équipe frappe plus fort et gagne de la jauge.');}
