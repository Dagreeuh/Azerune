...
export function castSkill(battle,index,targetId){
  ...
  if(e==='mistStrike'){const blesse=allies.filter(x=>!x.dead).sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp)[0];if(blesse)heal(blesse,actor.atk*(resonanceIV?.75:.62)*(1+mastery.power));}
  if(e==='renewingMist'){chosen.buffs.regen={turns:3+mastery.duration};heal(chosen,actor.atk*(resonanceIV?1.05:.88)*(1+mastery.power));const retires=cleanseUnit(chosen,1);if(retires.length)logs.push(`Brume revigorante purifie ${retires.map(cle=>DEBUFF_LABELS[cle]||cle).join(', ')} sur ${chosen.name}.`);}
  if(e==='revival'){let purifies=0;allies.filter(x=>!x.dead).forEach(x=>{heal(x,actor.atk*(resonanceIV?.92:.78)*(1+mastery.power));purifies+=cleanseUnit(x,resonanceIV?3:2).length;});logs.push(purifies?`Renouveau soigne lՎquipe et purifie ${purifies} malus.`:'Renouveau soigne toute lՎquipe.');}
