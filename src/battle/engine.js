...
export function nextTurn(battle){
  ...
  if(unit.buffs.healingSeed&&unit.hp/unit.maxHp<=.5){const amount=Math.round(unit.buffs.healingSeed.power||unit.maxHp*.2);healing+=amount;const sourceId=unit.buffs.healingSeed.source;cleanseUnit(unit,1);delete unit.buffs.healingSeed;const sylven=allies.find(value=>value.id===sourceId);if(sylven){sylven.mechanic.value=allies.filter(value=>value.id!==unit.id&&value.buffs?.healingSeed?.source===sourceId).length;sylven.mechanic.active=sylven.mechanic.value>0;}periodic.push(`Graine +${amount}`);}
...
export function castSkill(battle,index,targetId){
  ...
  if(e==='furyStrike'&&damageTotal>0){const rendu=heal(actor,Math.round(damageTotal*(resonanceIV?.26:.20)));if(rendu)logs.push(`${actor.name} se repaît du sang versé : ${rendu} PV récupérés.`);}
  if(e==='furyRecklessness'){actor.buffs.atkUp={turns:3+mastery.duration};const cout=Math.max(1,Math.round(actor.maxHp*.08));actor.hp=Math.max(1,actor.hp-cout);event(actor,cout,'recoil');logs.push(`Témérité : Attaque augmentée, ${cout} PV sacrifiés.`);}
